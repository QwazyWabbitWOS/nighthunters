...
void player_die(edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, vec3_t point)
{
	...
	if (!self->deadflag)
	{
		...
		// ***** Start of NH Changes *****
		if (attacker && attacker->client && !(attacker->client->resp.score >= fraglimit->value))
		{
			if (deathmatch->value)
...
void spectator_respawn(edict_t* ent)
{
	...
	if (ent->client->pers.spectator) {
		...
		//QW// value was unused char* value = Info_ValueForKey(ent->client->pers.userinfo, "spectator");
